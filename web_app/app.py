...
def build_rag_index(papers: list, session_id: str):
    ...
    for paper in papers:
        ...
        for i, chunk in enumerate(paper_chunks):
            ...
            metadatas.append({'doi': paper.get('doi', ''), 'title': paper.get('title', '')})
...
def get_general_rag_response(query: str, collection_name: str):
    ...
    try:
        ...
        print(f"ERROR: ChromaDB query failed for collection {collection_name}. Error: {e}")
        return "Sorry, I encountered an error while searching the documents. Please try another question."


    # 2. Join the retrieved chunks into a single, well-structured context block.
    context = "\n\n---\n\n".join(context_docs)
...
def handle_disconnect():
    ...
    if user_socket_id in USER_SESSIONS:
        ...
        print(f"Client disconnected: {user_socket_id}")
...
def _find_referenced_papers(query: str, papers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    ...
    for i, paper in enumerate(papers):
        ...
        for author_name in authors:
            ...
            if f"{last_name}'s paper" in query.lower() or f"({last_name})" in query.lower():
...
def handle_sidebar_chat(data):
    ...
    if referenced_papers:
        ...
        for i, paper in enumerate(referenced_papers, 1):
            injected_context += f"Referenced Paper {i} (Title: {paper.get('title', 'N/A')}):\n"
            injected_context += f"Abstract: {paper.get('abstract', 'No abstract available.')}\n\n"
    ...
    if context['type'] == 'general':
        if not rag_collection_name:
            ...
            response_text = get_general_rag_response(final_query_for_llm, rag_collection_name)
            # --- END OF FIX ---
            
    elif context['type'] == 'specific' and context.get('doi'):
        active_paper = next((p for p in final_papers if p.get('doi') == context['doi']), None)
...
def _generate_and_refine_keywords(title: str, emit_progress: callable) -> Dict[str, Dict[str, List[str]]]:
    """
...
def _collect_and_filter_papers(keywords: Dict, cutoff_year: int, emit_progress: callable) -> List[Dict]:
    """
    ...
    emit_progress("📚 **Phase 2: Collecting Academic Papers...** (This is the longest step)")
    ...
    emit_progress(f"-> Consolidated to {bucket_paper_count} papers before enrichment/filtering.")
    emit_progress(f"-> {len(collected_papers)} papers remain after enrichment and global filtering.")
    ...
    emit_progress(f"✅ Initial filtering complete. {len(papers_after_initial_filters)} papers remain.")

    return papers_after_initial_filters
    
def _generate_thematic_terms(title: str, emit_progress: callable) -> Tuple[List[str], List[str], List[str]]:
    """
    ...
    critiqued_domain_terms, domain_suggestions = critique_list(f"Domain terms for {title}", raw_domain_terms)
    ...
    critiqued_app_terms, app_suggestions = critique_list(f"Application terms for {title}", raw_app_terms)
    ...
    critiqued_tech_terms, tech_suggestions = critique_list(f"Technique terms for {title}", raw_tech_terms)
    ...
    return final_app_terms, final_tech_terms, final_domain_terms, list(app_suggestions.values()), list(tech_suggestions.values()), list(domain_suggestions.values())
...
def _rank_and_categorize_papers(
    ...
    if tech_terms and len(tech_terms) > 4:
        ...
        if len(valid_tech_terms) > 4:
            tech_term_vectors = [embed_text(term, use_cache=True) for term in valid_tech_terms]
            valid_tech_term_vectors = [vec for vec in tech_term_vectors if vec is not None]
            terms_actually_embedded = [term for term, vec in zip(valid_tech_terms, tech_term_vectors) if vec is not None]

            if valid_tech_term_vectors and len(valid_tech_term_vectors) > 1:
                emit_progress("-> Finding representative technique terms via clustering...")
                try:
                    ...
                    representative_tech_terms = list(set(terms_actually_embedded))[:10] # Cap for performance
                    emit_progress(f"-> Clustered terms into {num_clusters} groups. Using top representatives.")
                except Exception as e_cluster:
                    emit_progress(f"⚠️ Warning: Technique term clustering failed: {e_cluster}")
    ...
    emit_progress(f"-> Semantic ranking complete for {len(semantically_ranked_papers)} papers.")

    # Prepare regex with the CORRECT, expanded term lists
    app_regex = [re.compile(rf"\b{re.escape(term)}\b", re.IGNORECASE) for term in app_terms_for_patterns if term]
    tech_regex = [re.compile(rf"\b{re.escape(term)}\b", re.IGNORECASE) for term in representative_tech_terms if term]
    ...
    domain_regex = [re.compile(rf"\b{re.escape(term)}\b", re.IGNORECASE) for term in domain_terms_for_patterns if term]
    ...
    if len(focused_papers) < DESIRED_FOCUSED_COUNT:
        for p in semantically_ranked_papers:
            if len(focused_papers) >= DESIRED_FOCUSED_COUNT: break
            if p.get('doi') not in {fp.get('doi') for fp in focused_papers} and check_matches_app(p) and check_matches_tech(p):
                focused_papers.append(p)
    ...
    if len(focused_papers) < DESIRED_FOCUSED_COUNT:
        for p in semantically_ranked_papers:
            if len(focused_papers) >= DESIRED_FOCUSED_COUNT: break
            if p.get('doi') not in {fp.get('doi') for fp in focused_papers} and count_domain_hits(p) >= 1 and (check_matches_app(p) or check_matches_tech(p)):
                focused_papers.append(p)
    ...
    emit_progress(f"✅ Categorization complete: {len(final_focused_papers)} Focused, {len(final_exploratory_papers)} Exploratory.")
...
def _enrich_papers_with_insights(
    ...
    for i, paper in enumerate(papers_list):
        ...
        if abstract and isinstance(abstract, str) and abstract.strip():
            ...
            paper['insights_summary'] = "Insight generation skipped: Abstract was missing."
            paper['insights_relevance'] = "Relevance assessment skipped: Abstract was missing."
...
def run_pipeline(title: str, cutoff_year: int, user_socket_id: str):
    ...
    try:
        with tee_stdout(emit_log):
            ...
            emit_progress(f"🚀 **Starting full literature search for:** '{title}' (papers ≥ {cutoff_year})")
            ...
            app_terms, tech_terms, domain_terms, app_sug, tech_sug, domain_sug = _generate_thematic_terms(title, emit_progress)

            # Step 4: Rank & Categorize Papers - pass the suggestions
            focused_papers, exploratory_papers = _rank_and_categorize_papers(
                papers, title, app_terms, tech_terms, domain_terms, app_sug, tech_sug, domain_sug, emit_progress
            )
            ...
            if index_built:
                ...
                emit_progress("✅ RAG memory is ready. The 'General Chat' tab is now enabled.")
            else:
                USER_SESSIONS[user_socket_id] = {
                    "papers": all_final_papers,
                    "rag_collection_name": None
                }
                emit_progress("⚠️ RAG memory could not be built. 'General Chat' will be disabled.")
            ...
            emit_progress("🧠 **Finalizing: Building RAG memory for General Chat...**")

            # Create a unique, session-specific name for our ChromaDB collection.
            # This is crucial for separating different users' or different searches' data.
            session_collection_name = f"rag_collection_{uuid.uuid4().hex}"

            # Call the function from vector_store_manager.py to build the index.
            index_built = build_rag_index(
                collection_name=session_collection_name,
                papers=all_final_papers
            )
            ...
            USER_SESSIONS[user_socket_id].setdefault('chat_history', []).append({'sender': 'bot', 'message': intro_msg})

            socketio.emit('results_ready', {
                'papers': all_final_papers,
                'rag_collection_name': session_collection_name,
                'initial_chat_history': USER_SESSIONS.get(user_socket_id, {}).get('chat_history', []),
        ...
        emit_progress(f"❌ **An unexpected error occurred:**\n{str(e)}\nPlease try 'reset'.")
