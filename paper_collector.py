...
# # For Gemini API client management
...
Given a research paper title, generate EXACTLY 4 distinct academic topic labels that best frame the “Related Work” section. You must include:
  • 2 Core topics—directly at the heart of the title’s domain. These core topics MUST include ATLEAST 3 of the most relevant subject keywords from the title, and MUST include the subject matter.   
  • 1 Adjacent topic—close sibling areas that often cross-pollinate.  This Adjacent topic MUST include ATLEAST 1 of the most relevant subject keywords from the title, and MUST include the subject matter. Do not drift to a generic topic that omits the subject.
  • 1 Emerging topic—a nascent or hot area on the horizon related to the core subject matter. Do not drift to a generic topic that omits the subject.  
...
**Example 1:**
Research Title: "The Effects of Increased Creatine Consumption on Brain Function"
Correct Output:
Creatine Metabolism in the Brain (Core): Examining creatine's role in cerebral energy production and synthesis.
Cognitive Enhancement via Creatine (Core): Assessing the impact of creatine supplementation on memory and executive function.
Nutritional Neuroscience (Adjacent): The influence of dietary compounds like creatine on neural pathways and brain bioenergetics.
Therapeutic Potential of Creatine (Emerging): Investigating creatine as a treatment for neurodegenerative diseases.

**Example 2:**
Research Title: "Attention-Based Deep Learning Models for Natural Language Understanding"
Correct Output:
Attention Mechanisms in NLP (Core): Analyzing the function of attention in sequence-to-sequence language models.
Transformer Architectures for NLU (Core): Evaluating the performance of attention-based transformers on language tasks.
Computational Linguistics (Adjacent): Applying deep learning and attention models to understand language structure.
Explainable AI for Attention Models (Emerging): Developing methods to interpret and visualize attention layers in deep learning.

**Example 3:**
Research Title: "CRISPR-Cas9 Gene Editing for the Treatment of Sickle Cell Anemia"
Correct Output:
CRISPR-Cas9 for Hematological Disorders (Core): Targeting the HBB gene with CRISPR-Cas9 to correct sickle cell mutations.
Gene Therapy using CRISPR-Cas9 (Core): Utilizing CRISPR-Cas9 as a vector for in vivo and ex vivo cell therapies.
Molecular Basis of Sickle Cell Disease (Adjacent): Understanding the genetic pathology targeted by CRISPR-Cas9.
Off-Target Effects of CRISPR-Cas9 (Emerging): Assessing the safety and unintended genomic alterations of CRISPR-Cas9 therapies.

**Example 4:**
Research Title: "The Impact of Universal Basic Income on Poverty Reduction in Urban Environments"
Correct Output:
UBI and Economic Inequality (Core): Measuring the effect of Universal Basic Income programs on income distribution.
Implementation of UBI Pilots (Core): Studying the design and results of Universal Basic Income trials in cities.
Social Safety Nets (Adjacent): Comparing Universal Basic Income to traditional welfare and poverty alleviation programs.
UBI and the Future of Work (Emerging): Investigating the long-term effects of Universal Basic Income on labor markets.

**Example 5:**
Research Title: "Phytoremediation Techniques for Heavy Metal Contamination in Soil"
Correct Output:
Heavy Metal Uptake in Phytoremediation (Core): Analyzing mechanisms of heavy metal absorption by hyperaccumulator plants.
Applications of Phytoremediation (Core): Deploying specific plant species for phytoremediation of contaminated industrial sites.
Soil Chemistry and Bioavailability (Adjacent): How soil properties affect heavy metal availability for phytoremediation.
Genetic Engineering for Phytoremediation (Emerging): Enhancing heavy metal tolerance and accumulation in plants.
...
def collect_papers(
    ...
    Returns:
        Tuple containing:
            - The final list of unique, valid papers.
            - The total number of raw paper entries fetched before any filtering.
            - The number of papers after bucket grouping but before final enrichment/filters.
    """
    ...
    return final_unique_papers_list, total_raw_fetches, len(all_papers_collected_flat)
